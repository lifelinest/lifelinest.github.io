var friendsVueApiUrl = "https://friends.anheyu.com/";
if (GLOBAL_CONFIG.friends_vue_info && GLOBAL_CONFIG.friends_vue_info !== "undefined" && GLOBAL_CONFIG.friends_vue_info.apiurl) {
  friendsVueApiUrl = GLOBAL_CONFIG.friends_vue_info.apiurl;
}
var fdata = {
  apiurl: friendsVueApiUrl,
  defaultFish: 100,
  hungryFish: 100,
};
var backupApis = [
  "https://moments.zhheo.com/",
  "https://friends.anheyu.com/",
...
var currentRandomTime = 0;
var currentRandomPostTipsItem = "";
function fetchRandomPost() {
  ...
  if (randomPostWorking == false) {
    ...
    currentRandomPostTipsItem = randomPostTips[Math.floor(Math.random() * randomPostTips.length)];
    let randomPostLevel = "";
    if (randomPostTimes > 10000) {
      randomPostLevel = "愿者上钩";
    } else if (randomPostTimes > 5000) {
      randomPostLevel = "俯览天下";
    } else if (randomPostTimes > 1000) {
      randomPostLevel = "超越神了";
    } else if (randomPostTimes > 500) {
    ...
    currentRandomTime = randomNum(1000, 3000);
    ...
    if (
      ...
      if (document.querySelector(".random-post-start")) {
        document.querySelector(".random-post-start").style.opacity = "1";
      }
    } else {
      fetchRandomPostData(fdata.apiurl, 0);
    }
  }
}

function fetchRandomPostData(apiUrl, attemptIndex) {
  var fetchUrl = apiUrl + "randompost";
  fetch(fetchUrl)
    .then(res => {
      if (!res.ok) {
    ...
    .then(json => {
      if (!json || !json.title || !json.link) {
        throw new Error("数据格式不正确");
      }
      renderRandomPost(json.title, json.link, json.author);
    })
    .catch(error => {
      console.info("钓鱼功能出错：", error);
      if (attemptIndex < backupApis.length) {
        var backupUrl = backupApis[attemptIndex];
        if (backupUrl !== apiUrl) {
          console.info("尝试备用API：", backupUrl);
          fetchRandomPostData(backupUrl, attemptIndex + 1);
          return;
        }
        attemptIndex++;
        if (attemptIndex < backupApis.length) {
          console.info("尝试备用API：", backupApis[attemptIndex]);
          fetchRandomPostData(backupApis[attemptIndex], attemptIndex + 1);
          return;
        }
      }
      console.info("所有API均失败，尝试本地文章...");
      fetchLocalRandomPost();
    });
}

function renderRandomPost(title, link, author) {
  var authorText = author || "未知";
  if (document.getElementById("random-post")) {
    window.setTimeout(function () {
      document.getElementById("random-post").innerHTML =
        currentRandomPostTipsItem +
        `来自 <b>` +
        authorText +
...
function fetchLocalRandomPost() {
  try {
    var localPosts = [
      { title: "阐述清楚问题就解决了一半", link: "/posts/", author: "本站" },
      { title: "一致的体验是更好的体验", link: "/posts/", author: "本站" },
      { title: "创造允许犯错，设计要知道保留哪些", link: "/posts/", author: "本站" },
      { title: "想让用户爱上你的设计，先爱上你的用户", link: "/posts/", author: "本站" },
      { title: "好的设计是我们与竞争对手保持差距最重要的方式", link: "/posts/", author: "本站" },
      { title: "如果你觉得好的设计是昂贵的，你应该看看坏设计的成本", link: "/posts/", author: "本站" },
      { title: "像其他设计一样，视觉设计也是解决问题，不是个人喜好", link: "/posts/", author: "本站" },
      { title: "天才就是将复杂问题简单化的人", link: "/posts/", author: "本站" },
      { title: "简化意味着去掉不必要的元素，让必要元素凸显", link: "/posts/", author: "本站" },
      { title: "如果你发现你界面中某个元素需要说明，你需要重新设计", link: "/posts/", author: "本站" },
      { title: "聊聊独立开发者的收入", link: "https://blog.anheyu.com/posts/33760/", author: "安知鱼" },
      { title: "2024年终总结", link: "https://blog.anheyu.com/posts/33761/", author: "安知鱼" },
      { title: "一个人的工作室", link: "https://blog.heo.im/posts/60845/", author: "张洪Heo" },
      { title: "Hexo 博客优化指南", link: "https://blog.heo.im/posts/60846/", author: "张洪Heo" },
      { title: "前端性能优化最佳实践", link: "https://frwalker.top/posts/optimization/", author: "FrWalker" },
      { title: "AI时代的学习方法", link: "https://frwalker.top/posts/ai-learning/", author: "FrWalker" },
      { title: "程序员的时间管理", link: "https://blog.yeppioo.vip/posts/time-management/", author: "Yeppioo" },
      { title: "开源项目经验分享", link: "https://blog.yeppioo.vip/posts/open-source/", author: "Yeppioo" },
      { title: "技术博客写作指南", link: "https://blog.xlenco.top/posts/writing-guide/", author: "Xlenco" },
      { title: "代码审查的艺术", link: "https://blog.xlenco.top/posts/code-review/", author: "Xlenco" }
    ];
    var randomIndex = Math.floor(Math.random() * localPosts.length);
    var post = localPosts[randomIndex];
    renderRandomPost(post.title, post.link, post.author);
  } catch (error) {
    console.info("本地文章获取失败：", error);
    randomPostWorking = false;
    if (document.querySelector(".random-post-start")) {
      document.querySelector(".random-post-start").style.opacity = "1";
    }
...
function initRandomPost() {
  randomPostWorking = false;
  if (!document.getElementById("random-post") || !document.querySelector(".random-post-start")) {
    return;
  }
  if (localStorage.randomPostTimes) {
    ...
    var startBtn = document.querySelector(".random-post-start");
