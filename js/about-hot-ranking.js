 ...
 * 关于页 - 阅读排行 + 访客地图 渲染逻辑
 *
 * 数据来源：51.la → Cloudflare Worker 代理（stats.mfyresume.dpdns.org）→ 客户端渲染
 * 为什么不用 workers.dev：国内被 DNS 污染，实测 HTTP 000 不可达（2026-09-22 迁到自定义域）
 * 失败降级：本地缓存 → 显示友好错误提示
 *
...
(function () {
  ...
  // =============== 配置区 ===============
  // 主 Worker 地址。⚠️ 必须用自定义域，不能用 workers.dev：
  //   workers.dev 子域在国内被 DNS 污染，实测 HTTP 000 完全不可达，
  //   结果是「阅读排行 / 访客地图」两块对国内访客直接白掉（2026-09-22 迁域）。
  // 推荐做法：不改这里，而是在 _config.anzhiyu.yml 的 inject.head 中加一行：
  //   <script>window.__LA_STATS_PROXY__='https://stats.mfyresume.dpdns.org';</script>
  // 这样升级主题/JS 文件时不会丢失配置
  var DEFAULT_PROXY = 'https://stats.mfyresume.dpdns.org';

  // 备用 Worker（主域异常时的兜底；留空则不做二次尝试）
  var FALLBACK_PROXY = '';
  ...
  function getProxy() {
    // ① 显式覆盖优先：在 _config.anzhiyu.yml 的 inject.head 里设
    //    window.__LA_STATS_PROXY__='https://...' 可临时切换（如指向本地 wrangler dev）
    ...
    // ② 默认走生产代理。
    //    本地预览（localhost:4000）也走生产代理 —— Worker 白名单已包含
    //    http://localhost:4000，因此本地能看到与线上一致的真实数据。
    //    （旧实现本地指向 http://127.0.0.1:8787 的 wrangler dev，本地未启动该服务时必然失败，
    //     导致「本地怎么试都不出数」，已弃用该默认值。）
  ...
  // ⚠️ 2026-09-23 故障修复（访客地图「数据加载不出来」）
  //    原实现直连 geo.datav.aliyun.com，但该阿里 OSS 启用了**防盗链**。对照实测：
  //      空 Referer → 200 ｜ Referer=datav.aliyun.com → 200 ｜ Referer=localhost → 200
  //      Referer=mfy-lifeline.com（正是线上访客）→ **403 Forbidden**
  //    浏览器跨域 fetch 必然携带 Referer，故线上必 403 → 容器显示「地图数据加载失败」。
  //    （历史测试为何全绿：curl 裸测无 Referer；本地用 localhost 被放行；CDP 用本地副本打桩根本没发请求。）
  //    修复：① 首选**站内同域** /geo/china.json（随站点发布，无第三方防盗链、无跨域依赖）；
  //          ② 站内取不到时再降级回 datav，并显式 referrerPolicy:'no-referrer' 绕过防盗链。
  var CHINA_MAP_LOCAL_URL = '/geo/china.json';
  var CHINA_MAP_REMOTE_URL = 'https://geo.datav.aliyun.com/areas_v3/bound/100000_full.json';

  /**
   * 省份名归一化。
   * ⚠️ datav 的 GeoJSON 用**全称**（「广东省」「内蒙古自治区」「香港特别行政区」），
   *    而 51.la 返回**简称**（「广东」「内蒙古」「香港」）→ 不归一则名称 0 命中：
   *    地图能画出来、但任何省份都不会着色（看着像"数据没加载"，实为名称对不上）。
   */
  function shortProvinceName(n) {
    return String(n || '')
  ...
  function normalizeGeoNames(geoJson) {
    if (!geoJson || !geoJson.features) return geoJson;
    for (var i = 0; i < geoJson.features.length; i++) {
      var f = geoJson.features[i];
      if (f && f.properties && f.properties.name) {
        f.properties.name = shortProvinceName(f.properties.name);
      }
    }
    return geoJson;
  }
  ...
  function ensureChinaMapLoaded(callback) {
    ...
    // 站内同域优先 → 取不到再降级 datav（no-referrer 绕防盗链）
    fetch(CHINA_MAP_LOCAL_URL, { credentials: 'omit' })
      .then(function (res) {
        if (!res.ok) throw new Error('HTTP ' + res.status + ' @ local');
        return res.json();
      })
      .catch(function (e1) {
        console.info('[about-stats] local geojson failed, fallback to datav:', e1 && e1.message);
        return fetch(CHINA_MAP_REMOTE_URL, { credentials: 'omit', referrerPolicy: 'no-referrer' })
          .then(function (res) {
            if (!res.ok) throw new Error('HTTP ' + res.status + ' @ datav');
            return res.json();
          });
      })
      .then(function (geoJson) {
        if (typeof echarts === 'undefined') throw new Error('echarts not ready');
        // 名称归一后再注册，否则 51.la 的简称与 GeoJSON 全称匹配不上、省份不着色
        echarts.registerMap('china', normalizeGeoNames(geoJson));
        chinaMapLoaded = true;
  ...
  function ensureMapChart(el) {
    ...
    // 容器高度由 flex 决定，可能在 init 之后才稳定（字体/图片加载、Live2D 注入等）
    // → 只靠一次性 resize 会得到偏小的 canvas（实测 440 高的容器只画出 200 高）。
    //   用 ResizeObserver 持续跟随，保证地图铺满卡片。
  ...
  function renderMapChartInner(el, list) {
    ...
    var option = {
      ...
      visualMap: {
        ...
        max: maxVal || 100,
        left: 'left',
        // 抬高 36px：贴 bottom 会与卡片底部说明文字「近 N 天访客省份分布」挤在一起
        bottom: 36,
        text: ['高', '低'],
        calculable: true,
